(function($) {
  ...
  function fullscreen_slider() {
	...
	/* 
	 *
	 * Here we begin a long and confusing initialization of some sort
	 * 
	 * 
	 */
  fullscreen_slider.prototype.init = function(node, settings) {
    ...
    caller.current = 0;
    //caller.padding = $(window).height() * 0.15;
	//caller.padding = ($(window).height() - $(node).children('img').height) /2;
	...
	caller.image_height = $(node).children('img').height;//-caller.padding*2;
    caller.image_width = $(node).children('img').width;//-caller.padding*2;
    //caller.image_height = 600;
  ...
  fullscreen_slider.prototype.reset = function() {
	...
	//caller.padding = ($(window).height() - $(node).children('img').height) *.1;
    //var image_height = window_height - caller.padding * 2;
    //var image_width = image_height * 1.33;
	var image_height = $(node).children('img').height;//-caller.padding*2;
    var image_width = $(node).children('img').width;//-caller.padding*2;

	//Original
    caller.image_width = image_width;
    caller.image_height = image_height;
	
	//calling image from node
	//caller.image_height = caller.node.children('img').height;
	//caller.image_width = caller.node.children('img').width;
	...
	/*
	 * Variables are made for each state of the image
	 */
	...
	var curr = (current_child.width(image_width)-window_width)/2;
    current_child.css({
      "top": caller.padding,
